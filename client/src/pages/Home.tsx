...
function Sidebar({ page, setPage, onLogout, isOpen, onClose }: { page: Page; setPage: (page: Page) => void; onLogout: () => void; isOpen: boolean; onClose: () => void }) {
  ...
  return <aside className={`sidebar fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-blue-900/30 bg-[#0c1228]/95 p-4 backdrop-blur-2xl transition-transform duration-200 ease-out md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
    ...
    <div className="mt-auto space-y-4">
      ...
      <div className="flex items-center gap-3 border-t border-white/5 px-2 pt-4"><div className="grid size-9 place-items-center rounded-full bg-gradient-to-br from-violet-400 to-blue-600 text-xs font-black text-white">FT</div><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold text-white">Workspace member</div><div className="truncate text-[11px] text-slate-500">Secure account</div></div><div className="flex items-center gap-1"><button aria-label="Sign in" onClick={() => startLogin()} className="rounded-lg px-2 py-1.5 text-xs font-semibold text-blue-300 transition hover:bg-blue-500/10 hover:text-blue-200">Sign in</button><button aria-label="Sign out" onClick={onLogout} className="flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-xs font-semibold text-slate-500 transition hover:bg-white/5 hover:text-white"><LogOut className="size-4" /><span>Sign out</span></button></div></div>
    </div>
  </aside>;
}
...
function WalletPage({ setPage: _setPage }: { selectedPlan?: string; selectedAmount?: number; setPage: (page: Page) => void }) {
  ...
  const payNow = () => {
    ...
    if (!paystack || paymentLoadError) {
      toast.error(paymentLoadError ? "Secure payment is unavailable. Try again shortly." : "Payment is still loading. Please try again.");
      setPaying(false);
      return;
    }
    try {
      ...
      setPaying(false);
      toast.error("Secure payment could not open. Please try again.");
    }
  };
